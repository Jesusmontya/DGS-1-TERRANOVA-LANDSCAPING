...
type PageProps = {
  ...
  showSiteChrome?: boolean; photos?: string[]; contextSection?: { title: string; text: string; cta: string }
}
...
export default function CommercialServicePage({ canonical, serviceName, eyebrow, title, intro, photoAlt, tags, features, faqs, related, showSiteChrome = false, photos = realPhotos, contextSection }: PageProps) {
  const serviceSchema = {
  ...
  return <main className={styles.page} style={{ '--hero': `url('${photos[0]}')`, '--photo': `url('${photos[1] || photos[0]}')` } as CSSProperties}>
